...
<!-- HERO demo GIF / video — blog_with_review 端到端含 interrupt/resume。
     文件：docs/images/hero-demo.gif  (1280×720, ≤6MB, 10–15s loop) -->
...
**同一管线三种执行模式** —— 批处理管线运行 · 普通聊天 · 自主聊天（`coordinator` 调 `spawn_agent` / `start_project_run`）· ClawBot 群聊（Discord / QQ / WeChat）带意图路由与两阶段确认。
...
<!-- 图：三个入口 → 一个 SessionRunner → 共享 agent loop → tools / memory / telemetry
     文件：docs/images/three-drivers.svg -->
...
**1. `micro_compact` —— 每轮执行，不花 LLM。** 最便宜的一道。每次 agent 循环迭代进入时，除了最近 `keep_recent` 条（默认 3 条）之外的 tool-result 消息，其 `content` 会被替换为 `[Old tool result cleared]`。一次读取密集的 researcher 轮次跑了十次 `search_docs`，最近的三条 body 保留完整，另外七条缩成一个标记串。免费、确定性、不需要调用总结器。
...
<!-- IMAGE: 仅追加式压缩时间线 —— 消息增长 → 追加 summary + boundary → 切片视图
     文件：docs/images/compact-timeline.svg -->
...
<!-- IMAGE: Observability 页面 + Grafana 仪表板，2×1 组合
     文件：docs/images/observability.png -->
...
pipelines/     5 条 YAML 管线（blog × 2、courseware_exam、tests × 2）
