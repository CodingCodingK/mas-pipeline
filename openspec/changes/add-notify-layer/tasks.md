...
- [ ] 1.1 Add `user_notify_preferences` table to `scripts/init_db.sql`: `(user_id INT NOT NULL, event_type TEXT NOT NULL, channels JSONB NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY (user_id, event_type))` + index on `(user_id)` + FK to `users(id)`
- [ ] 1.2 Add `NotifySettings` pydantic model to `src/project/config.py` with fields: `enabled: bool = True`, `wechat_webhook_url: str | None = None`, `discord_webhook_url: str | None = None`, `sse_queue_size: int = 500`, `sse_heartbeat_sec: int = 15`, `notify_queue_size: int = 5000`; wire into top-level `Settings` under `notify:` key

## 2. Notify core

- [ ] 2.1 Create `src/notify/__init__.py` exporting `Notifier`, `NullNotifier`, `Notification`
- [ ] 2.2 Create `src/notify/events.py` with `Notification` dataclass: `event_type: Literal[...]`, `user_id: int`, `title: str`, `body: str`, `payload: dict`, `notification_id: str`, `created_at: datetime`; include a `to_dict()` helper for channel serialization
- [ ] 2.3 Create `src/notify/rules.py`:
...
- [ ] 2.4 Create `src/notify/preferences.py`:
...
- [ ] 2.5 Create `src/notify/channels/__init__.py` exporting `Channel`, `SseChannel`, `WechatChannel`, `DiscordChannel`
- [ ] 2.6 Create `src/notify/channels/base.py` — `Channel` Protocol: `name: str` attribute + `async deliver(notification: Notification) -> None`
- [ ] 2.7 Create `src/notify/channels/sse.py`:
...
- [ ] 2.8 Create `src/notify/channels/wechat.py`:
...
- [ ] 2.9 Create `src/notify/channels/discord.py`:
  ...
  - `async close()` to close the httpx client
- [ ] 2.10 Create `src/notify/notifier.py`:
...
- [ ] 4.1 Write `scripts/test_notify_rules.py`:
...
- [ ] 4.2 Write `scripts/test_notify_channels.py`:
...
- [ ] 4.3 Write `scripts/test_notifier.py`:
...
- [ ] 4.4 Write `scripts/test_notify_api.py` (FastAPI TestClient, mocked deps):
...
- [ ] 5.1 Write `scripts/test_notify_integration.py`:
...
- [ ] 5.2 Write `scripts/test_notify_rest_integration.py`:
...
- [ ] 6.1 Run `scripts/test_event_bus.py` (change #1 tests) — must still pass
- [ ] 6.2 Run full telemetry test surface (6 scripts) — must still pass
- [ ] 6.3 Run `scripts/test_session_runner.py`, `scripts/test_streaming_regression.py`, `scripts/test_claw_gateway.py`, `scripts/test_bus_session_runner_integration.py` — must still pass
- [ ] 6.4 Run `scripts/test_rest_api_integration.py`, `scripts/test_rest_api_auth.py`, `scripts/test_rest_api_sse_backfill.py` — notify router mount must not break existing routes

## 7. Validation and archive

- [ ] 7.1 Run `openspec validate add-notify-layer --strict`
- [ ] 7.2 Run the full notify test surface: `test_notify_rules.py`, `test_notify_channels.py`, `test_notifier.py`, `test_notify_api.py`, `test_notify_integration.py`, `test_notify_rest_integration.py`
- [ ] 7.3 Run regression suite (task 6.1–6.4)
- [ ] 7.4 Update `.plan/progress.md`: mark Phase 6.3 done, next step = Phase 6.4 (Web frontend)
- [ ] 7.5 `git add` + commit
- [ ] 7.6 Run `/openspec-archive-change add-notify-layer`
