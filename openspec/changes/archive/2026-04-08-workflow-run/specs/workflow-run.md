...
`create_run(project_id, session_id=None, pipeline=None)` SHALL insert a row into workflow_runs with a generated unique run_id, status='pending', started_at=now(), and sync to Redis.
