...
`create_run(project_id, session_id=None, pipeline=None)` SHALL insert a row into workflow_runs with a generated unique run_id, status='pending', started_at=now(), and sync to Redis. The `pipeline` field SHALL store the pipeline name when the run is associated with a pipeline execution. When called from `run_coordinator`, the pipeline field SHALL be set based on the Project's pipeline configuration.
